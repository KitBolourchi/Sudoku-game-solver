...
class Grid:
    ...
    def __init__(self, rows, cols, width, height, win):
        self.rows = rows
        self.cols = cols
        self.cubes = [[Cube(self.board[i][j], i, j, width, height) for j in range(cols)] for i in range(self.rows)]
        self.width = width
        self.height = height
        self.model = None
        self.update_model()
...
def redraw_window(win, board, time, strikes):
    ...
    # draw strikes
    text = fnt.render(f'Incorrect Guesses: {strikes}', 1, (255, 0, 0))
